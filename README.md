...
`GET /api/runs/{id}/break/{n}` returns the reviewable items — themes, gaps,
seminal works, feasibility verdicts — plus the directive vocabulary. The UI
renders widgets; submitting sends `directives` (generated by those widgets)
alongside free text. Both are combined into the **same directive language the
CLI uses**, so a run answered in a browser is indistinguishable downstream from
one answered at a terminal.

A break is also where you can change models for the agents that have not run
yet — send `model_overrides` with the submission.
